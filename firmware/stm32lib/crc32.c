...
static int _crc32_isTableEmpty = 1;
static uint32_t _crc32_table[256];

static void _crc32_makeTable();
 ...
 */
const uint32_t* _crc32_getTable() {
#ifdef DYNAMIC_CRC_TABLE
  if (_crc32_isTableEmpty) {
    _crc32_makeTable();
  }
#endif
  return (const uint32_t*)_crc32_table;
}

/* ========================================================================= */
#define DO1(buf) crc = _crc32_table[((int)crc ^ (*buf++)) & 0xff] ^ (crc >> 8);
